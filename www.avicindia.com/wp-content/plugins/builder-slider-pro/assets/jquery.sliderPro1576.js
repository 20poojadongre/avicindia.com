...
;(function( window, $ ){
	...
	SliderPro.prototype ={
		...
		update(){
			...
			this.middleSlidePosition = parseInt( ( this.slidesOrder.length - 1 ) / 2, 10 );
		...
		_updateSlidesPosition(){
			const selectedSlidePixelPosition = parseInt( this.$slides.find( '.sp-slide' ).eq( this.selectedSlideIndex ).css( this.positionProperty ), 10 );
		...
		_resetSlidesPosition(){
			...
			const newSlidesPosition = - parseInt( this.$slides.find( '.sp-slide' ).eq( this.selectedSlideIndex ).css( this.positionProperty ), 10 ) + this.visibleOffset;
		...
		gotoSlide( index ){
			...
			// Calculate the new position that the slides container need to take
			 newSlidesPosition = - parseInt( this.$slides.find( '.sp-slide' ).eq( this.selectedSlideIndex ).css( this.positionProperty ), 10 ) + this.visibleOffset;
	...
	TransitionEffects ={
			...
			if ( this.$slider.hasClass( 'sp-swiping' ) ){
				...
				if ( this.settings.fadeOutPreviousSlide === true ){
					this._transition_effect( $previousSlide, 'fadeOut', transition );
                                ...
                                const _transition_callback = function(){
					// Reset the position of the slides and slides container
					that._resetSlidesPosition();

					// Wait before reposition is running
					const waitingTime = parseFloat( $nextSlide.data( 'duration' ) ) * 1000;
		...
		_transition_effect( target, effect, duration, callback ){
			...
			target.on( this.transitionEvent, function( e ){
					...
					$(this).off( that.transitionEvent ).css( 'transition', '' );
