(function ($,Themify) {
    ...
    _lazyLoading=function(el,self){
            ...
            if(div){
                    const attr = Themify.parseVideo(div.getAttribute('data-url')),
				...
				if(video){
					if(video.preload==='none'){
                        ...
                        if(self.settings.autoplay===true && self.isTimerRunning===true){
				...
				const item=instance.elements[0],
					...
					config = {
						...
						gotoSlide:function(e){
							if(e.index===this.slides.length-1 && this.options.autoplay===true && pasue_last){
                                this.stopAutoplay();
                            }
						    for(let j=0;j<2;++j){
								let index=e.index+j,
